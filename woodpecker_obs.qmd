...
tab_combined <- pivoted_data %>%
  group_by(Species) %>%
  summarise(across(`1`:`32`, sum, na.rm = TRUE))
...
data_encoded <- cbind(
  df[, c("X15_time_spent_sec")],  # Keep the continuous columns
  model.matrix(~ X9_Foraging_type - 1, df), # One-hot encoding for 'species'
  model.matrix(~ X10_Substrate - 1, df), # One-hot encoding for 'condition'
  model.matrix(~ X11_where - 1, df) # One-hot encoding for 'condition'
