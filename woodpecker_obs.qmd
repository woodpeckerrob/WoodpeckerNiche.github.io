...
hyper_df <- df %>% dplyr::select("X8_Species","X3_Plot_number","month_num","X10_Foraging_type","X11_Substrate","X12_where","X13_if_dead_decay_cla","X15_tree_size_class","X22_height_ratio","X16_time_spent_sec")
colnames(hyper_df) <- c("species","plot","month","foraging_type","substrate","where","decay_class","tree_size","height_ratio","time")
...
data.scores$sp <- hyper_data_weighted$species  #  create a column of ocean names, from the original data frame 
data.scores$plot <- hyper_data_weighted$plot
...
pcoa_plot <- ggplot(data.scores, aes(x= Axis.1, y= Axis.2, col=sp)) + #denote groupings by color "col" and shape
  geom_point(aes(size=plot)) +#adds points
  #geom_text(aes(label=rownames(data.scores)),hjust=0, vjust=0)+#adds island names
  #stat_ellipse() +#adds ellipses
...
pcoa_plot_for <- ggplot(data.scores, aes(x= Axis.1, y= Axis.2, col=sp)) + #denote groupings by color "col" and shape
  geom_point(aes(size=plot)) +#adds points
  #geom_text(aes(label=rownames(data.scores)),hjust=0, vjust=0)+#adds island names
  #stat_ellipse() +#adds ellipses
  geom_convexhull(aes(fill = sp), alpha = 0.1)+
  theme_bw() +
#  xlim(-1.5, 1.5)+
 # ylim(-1.5,1.5)+
  labs(title = "Foraging Behavior PCoA Plot",
       x = xlab,
       y = ylab)
