...
hyper_long <- hyper_df2 %>%
  unite("combo",  substrate, where, decay_class, sep = ".") %>% #foraging_type,
  select(species,combo, time, plot)
hyper_wide <- hyper_long %>%
  mutate(id = row_number()) %>%
  pivot_wider(names_from = combo, values_from = time, values_fill = 0 )%>%
  select(species,plot,id, where(~ is.numeric(.) && sum(.) > 0))
...
hyper_data$id <- NULL
hyper_data <- subset(hyper_data,hyper_data$species!="HAWO")  #remove HAWO for now
...
hyper_data_weighted <- hyper_data %>%
  ...
  select(species, plot, starts_with("pct_"))
...
data.scores$sp <- hyper_data_weighted$species  #  create a column of ocean names, from the original data frame 
plot_and_burn <- strsplit(hyper_data_weighted$plot, "_")#[[1]]
...
hyper_wide <- hyper_df %>%
  mutate(id = row_number()) %>%
  pivot_wider(names_from = foraging_type, values_from = time, values_fill = 0 )%>%
  select(species,plot,id, where(~ is.numeric(.) && sum(.) > 0))
...
data.scores <- as.data.frame(nmds_mod$points)  #Using the scores function from vegan to extract 
#                                                the site scores and convert to a data.frame
data.scores$sp <- hyper_data_weighted$species  #  create a column of species names, from the original data frame 
#head(data.scores)  #look at the data
