...
N_array <- aperm(out$N.samples, c(3, 2, 1))  # Now: [32, 13, 11250]

# Now compute posterior means: mean over the 3rd dimension (samples)
N_means <- apply(N_array, c(1, 2), mean)  # [32 sites x 13 species]
# 2. Create data frame
abund_df <- as.data.frame(N_means)
colnames(abund_df) <- out$sp.names

# Convert to long format
N_means_long <- as.data.frame(abund_df) %>%
  mutate(site_index = 1:nrow(.)) %>%
  pivot_longer(cols = -site_index,
               names_to = "species",
               values_to = "abundance")

# Add site info (ensure matching order)
site_info <- pc %>%
...
# Join
plot_df <- left_join(N_means_long, site_info, by = "site_index")
...
ggplot(plot_df, aes(x = factor(point_id), y = abundance, fill = species)) +
